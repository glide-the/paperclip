# Paperclip Issue Lifecycle Analysis
...
sequenceDiagram
    autonumber
    participant Board as 👤 看板用户
    participant Server as 🖥️ API Server
    participant DB as 🗄️ Database
    participant Agent as 🤖 Agent
    participant AuditLog as 📋 审计日志

    Note over Agent,DB: 原子签出：同一时刻最多一个 Agent 持有该 Issue
    Note over Board,Server: 终态：done / cancelled 不可直接转其他状态（需 reopen）

    rect rgb(232,242,255)
        Note over Board,Agent: 🟦 创建与分配（步骤 1-2）

        Board->>Server: 步骤1 Board 移入待办<br/>PATCH /issues/:id {status:'todo'}<br/>backlog -> todo
        Server->>DB: 更新 issue.status='todo'
        DB-->>Server: OK
        Server->>AuditLog: issue.updated (from=backlog,to=todo,actor=board)
        Server-->>Board: 200 status=todo

        Agent->>Server: 步骤2 Agent 原子签出<br/>POST /issues/:id/checkout<br/>todo -> in_progress
        Server->>DB: 条件更新(仅 status='todo' 且未被占用时成功)
        alt 签出成功（转 in_progress）
            DB-->>Server: row updated
            Server->>AuditLog: issue.checked_out (issueId,agentId)
            Server->>AuditLog: issue.updated (from=todo,to=in_progress,actor=agent)
            Server-->>Agent: 200 status=in_progress
        else 409 冲突（另一个 Agent 已签出）
            DB-->>Server: 0 rows updated
            Server->>AuditLog: issue.checkout_conflict (issueId,agentId)
            Server-->>Agent: 409 Conflict
        end
    end

    rect rgb(255,248,220)
        Note over Board,Agent: 🟨 执行与阻塞（步骤 3-7）

        Agent->>Server: 步骤3 Agent 上报阻塞<br/>PATCH /issues/:id {status:'blocked'}<br/>in_progress -> blocked
        Server->>DB: 更新 issue.status='blocked'
        DB-->>Server: OK
        Server->>AuditLog: issue.updated (from=in_progress,to=blocked,actor=agent)
        Server-->>Agent: 200 status=blocked

        Board->>Server: 步骤4 Board 解除阻塞<br/>PATCH /issues/:id {status:'todo'}<br/>blocked -> todo
        Server->>DB: 更新 issue.status='todo' 并清理占用者
        DB-->>Server: OK
        Server->>AuditLog: issue.released (issueId,previousAgentId)
        Server->>AuditLog: issue.updated (from=blocked,to=todo,actor=board)
        Server-->>Board: 200 status=todo

        Agent->>Server: 步骤5 Agent 再次签出<br/>POST /issues/:id/checkout<br/>todo -> in_progress
        Server->>DB: 条件更新(仅 status='todo' 且未被占用时成功)
        DB-->>Server: row updated
        Server->>AuditLog: issue.checked_out (issueId,agentId)
        Server->>AuditLog: issue.updated (from=todo,to=in_progress,actor=agent)
        ...
        Agent->>Server: 步骤6 Agent 提交评审<br/>PATCH /issues/:id {status:'in_review'}<br/>in_progress -> in_review
        Server->>DB: 更新 issue.status='in_review'
        DB-->>Server: OK
        Server->>AuditLog: issue.updated (from=in_progress,to=in_review,actor=agent)
        Server-->>Agent: 200 status=in_review

        Board->>Server: 步骤7 Board 打回修改<br/>PATCH /issues/:id {status:'in_progress'}<br/>in_review -> in_progress
        Server->>DB: 更新 issue.status='in_progress'
        DB-->>Server: OK
        Server->>AuditLog: issue.updated (from=in_review,to=in_progress,actor=board)
        Server-->>Board: 200 status=in_progress
    end

    rect rgb(232,255,236)
        Note over Board,Agent: 🟩 完成与异常（步骤 8-10）

        Agent->>Server: 步骤8 Agent 完成<br/>PATCH /issues/:id {status:'done'}<br/>in_progress -> done
        Server->>DB: 更新 status='done', completedAt=now, 清理占用者
        DB-->>Server: OK
        Server->>AuditLog: issue.released (issueId,agentId)
        Server->>AuditLog: issue.updated (from=in_progress,to=done,actor=agent,completedAt)
        Server-->>Agent: 200 status=done

        Board->>Server: 步骤9 Board 评论重开<br/>POST /issues/:id/comments {reopen:true}<br/>done -> todo
        Server->>DB: 事务: 插入评论 + status='todo' + 清空 completedAt
        DB-->>Server: OK
        Server->>AuditLog: issue.reopened (issueId,commentId,actor=board)
        Server->>AuditLog: issue.updated (from=done,to=todo,actor=board)
        Server-->>Board: 201 comment + 200 status=todo

        Board->>Server: 步骤10 Board 取消<br/>PATCH /issues/:id {status:'cancelled'}<br/>{todo|in_progress|blocked|in_review} -> cancelled
        Server->>DB: 更新 status='cancelled', cancelledAt=now, 清理占用者
        DB-->>Server: OK
        Server->>AuditLog: issue.released (if holder exists)
        Server->>AuditLog: issue.updated (to=cancelled,actor=board,cancelledAt)
        Server-->>Board: 200 status=cancelled
    end
```